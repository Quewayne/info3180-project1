...
from wtforms import StringField, SubmitField, FileField, RadioField, HiddenField, TextAreaField
...
class NameForm(Form):
    fle= FileField('Profile Pic', validators=[FileAllowed(['png', 'jpg', 'jpeg', 'gif'], 'Images only!')])
    ...
    if form.fle.data:
    	...
    	while True:
        	...
        	if not db.session.query(exists().where(Profile.uid == str(uid))).scalar():
            		break
    	profile=Profile(form.fname.data, form.lname.data, form.age.data,form.sex.data,im_fn,form.uname.data,form.bio.data, datet(),uid) 
...
def  showprofile(idn):
    ...
    user = {'id':profile.uid,'image':imgURL,'fname':profile.fname, 'lname':profile.lname,'age':profile.age, 'sex':profile.sex,'uname':profile.uname,'bio':profile.bio}
